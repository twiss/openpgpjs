...
Message.prototype.encrypt = async function(keys, passwords, sessionKey, wildcard=false, date=new Date(), userId={}) {
  ...
  if (sessionKey) {
    ...
    symAlgo = enums.read(enums.symmetric, await getPreferredSymAlgo(keys, date, userId));
  ...
  const msg = await encryptSessionKey(sessionKey, symAlgo, keys, passwords, wildcard, date, userId);
 ...
 * @async
 */
export async function encryptSessionKey(sessionKey, symAlgo, publicKeys, passwords, wildcard=false, date=new Date(), userId={}) {
