 ...
 * @param  {Date} date use the given date for verification instead of the current time
 * @param  {String} userId (optional) user ID to get instead of the primary user, if it exists
...
Key.prototype.getPrimaryUser = async function(date=new Date(), userId) {
  ...
  const primaryUser = this.users.map(function(user, index) {
    ...
    return (
      ((a.user.userId.userid === userId) - (b.user.userId.userid === userId)) ||
 ...
 * @async
 */
export async function getPreferredHashAlgo(key, date, userId) {
 ...
 * @async
 */
export async function getPreferredAlgo(type, keys, date, userId) {
 ...
 * @async
 */
export async function isAeadSupported(keys, date, userId) {
  let supported = true;
  // TODO replace when Promise.some or Promise.any are implemented
  await Promise.all(keys.map(async function(key) {
    const primaryUser = await key.getPrimaryUser(date, userId);
